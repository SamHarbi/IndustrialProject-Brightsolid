...
function createAudit(req, exep) {
    return new Promise((resolve, reject) => { //First check that exception is not compliant and then delete it
        ...
        connection.query(que, [exep[0].exception_id, req.session.accountID, req.body.ruleID, lastUpdate, exep[0].justification, req.body.justification, exep[0].reviewDate, reviewDate], (err, row, fields) => {
            if (err) { //Query didn't run
                reject(err);
            }
            else {
